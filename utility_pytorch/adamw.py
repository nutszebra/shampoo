...
class AdamW(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
                 weight_decay=0.05 * math.sqrt(64 / 50000)):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if len(state) == 0:
                    ...
                    # state['exp_avg_sq'] = torch.zeros_like(p.data)
                    state['exp_avg_sq'] = p.data.new(p.data.size()).zero_()
                    state['exp_avg_sq_max'] = p.data.new(p.data.size()).zero_()
                ...
                exp_avg_sq_max = torch.max(exp_avg_sq_max, exp_avg_sq)
                denom = exp_avg_sq_max.sqrt().add_(group['eps'])
